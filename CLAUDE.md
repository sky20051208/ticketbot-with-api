...
- **關鍵字全沒中時改用「目標價位」算期望值**（`desirability()` / `price_affinity()`）。
  使用者的習慣是**把價位打在關鍵字最後一順位**（`搖滾A;搖滾B;6800`），
  `target_price()` 自動認出來。這條很重要 —— 沒中的票區原本是照官方 `sortedIndex`
  排，名次衰減套在那個順序上等於**在跟使用者意圖無關的排序上加權**，而且價位剛好對
  但排第 50 的票區只有 `0.85^50 ≈ 0.0003`，等於永遠選不到。改成：沒中的一律先退到
  命中的後面當同一階（`RANK_DECAY^n_matched`），再由價位契合度連續拉開
  ```
  契合度：便宜 = price/target（線性折）；超價 = (target/price)^PRICE_OVER_PENALTY
  目標 6800 → 6500:0.96  3800:0.56  1800:0.26 ／ 8800:0.46  12000:0.18
  ```
  刻意不對稱（超價罰更重）：買貴了客人可能不認差額、變成自己吃掉，比坐差一點嚴重。
  **`PRICE_OVER_PENALTY=1.0` 就變成貴便宜一視同仁**。不是硬上限 —— 只剩超價的還是會買
- **價位認定有防呆**：只認純數字那組（`綠517`、`G05` 帶字母的不算），多組取最後一組，
  還會拿這場的真實票價區間健檢（`[min×0.5, max×2]`），離譜的數字當一般關鍵字用。
  實測 Vaundy（票價 800/2880/3880/4880/5880）打 `搖滾X;搖滾Y;5800` 關鍵字 0 命中，
  正確落在最接近的 5880（契合度 0.96）—— 子字串比對在這種「打的價位不完全等於任一區」
  的情況會整個沒中，這就是價位契合度要存在的理由
- **比較是全部攤平一起比，沒有分層** —— 所以同價位的票區必須自己破同分（`TIER_DECAY`，
  `_tier_ranks`）。少了這一項的話「藍1A~藍1E」價位契合度一模一樣、想要度也一模一樣，
  誰被選中就 100% 由張數決定，實測會挑到同價位裡**位置最差**的那一區。用 0.97 這種
  弱衰減而不是 `RANK_DECAY`，是因為場館把它們定成同一個價，本身就代表差不多，
  不該蓋過真實的張數差距（設 1.0 = 回到修之前、同價位純看張數）
